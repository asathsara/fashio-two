...
export const insertSubCategory = async (id, name) => {
    const response = await axiosInstance.post(API_ENDPOINTS.ADD_SUBCATEGORY(id), { name });
    return response.data;
}

export const deleteSubCategory = async (categoryId, subItemName) => {
    const response = await axiosInstance.delete(API_ENDPOINTS.DELETE_SUBCATEGORY(categoryId, subItemName));
    return response.data;
}   